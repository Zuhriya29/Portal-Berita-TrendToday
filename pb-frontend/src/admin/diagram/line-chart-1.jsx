...
export const LineChart1 = () => {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:8000/api/statistik-kategori")
      .then((response) => response.json())
      .then((result) => {
        // result: [ { kategori: "Nasional", total_pengunjung: 1200 }, ... ]
  ...
  return (
    <div style={{ height: "25rem", width: "100%" }}>
      <p style={{ textAlign: "center", fontSize: "1em", fontWeight: "bold", marginTop: "3vh" }}>
        Statistik Pengunjung per Kategori
      </p>
      <ResponsiveLine
        data={data}
        ...
        yScale={{
          type: "linear",
          min: 0,
          max: "auto", // biar dinamis
          stacked: false,
          reverse: false,
        }}
        yFormat={value => `${value} Pengunjung`}
        curve="monotoneX"
        axisTop={null}
        axisRight={null}
        axisBottom={{
